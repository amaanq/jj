...
use jj_lib::git_backend::WorkspaceColocation;
...
///
/// If `ui` is provided and the workspace has a `.git` worktree gitlink that
/// doesn't work (broken, or belonging to a different repo), a warning is
/// shown: that state usually means colocation silently stopped working. An
/// unrelated `.git` directory or symlink is treated as a deliberate separate
/// repo and stays silent here; `jj git init` and `jj git colocation status`
/// report those via [`report_unexpected_git_in_workspace()`].
pub fn is_colocated_git_workspace(ui: Option<&Ui>, workspace: &Workspace) -> bool {
    let Ok(git_backend) = git::get_git_backend(workspace.repo_loader().store()) else {
        return false;
    };
    match git_backend.check_workspace_colocation(workspace.workspace_root()) {
        WorkspaceColocation::Colocated => true,
        WorkspaceColocation::NoDotGit => false,
        WorkspaceColocation::UnrelatedDotGit => {
            if let Some(ui) = ui {
                let is_gitlink_file = workspace
                    .workspace_root()
                    .join(".git")
                    .symlink_metadata()
                    .is_ok_and(|meta| meta.is_file());
                if is_gitlink_file {
                    explain_unexpected_git_in_workspace(ui, workspace, git_backend);
                }
            }
            false
        }
...
/// Reports an unexpected `.git` presence in a non-colocated workspace.
///
/// Unlike the automatic warning in [`is_colocated_git_workspace()`], this
/// covers all cases including an unrelated `.git` directory or symlink. Meant
/// for explicit diagnosis points (`jj git init`, `jj git colocation status`).
pub fn report_unexpected_git_in_workspace(ui: &Ui, workspace: &Workspace) {
    let Ok(git_backend) = git::get_git_backend(workspace.repo_loader().store()) else {
        return;
    };
    explain_unexpected_git_in_workspace(ui, workspace, git_backend);
...
fn explain_unexpected_git_in_workspace(ui: &Ui, workspace: &Workspace, git_backend: &GitBackend) {
    ...
    if meta.is_dir() {
        ...
        writeln!(
            ui.hint_default(),
            "This Git repo is ignored by jj. If it is unintentional, remove it or move it out of \
             the workspace."
        )
        .ok();
        return;
