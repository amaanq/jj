...
fn test_colocated_workspace_git_symlink_to_wrong_repo() {
    ...
    // Regular commands stay silent about the unrelated .git symlink; it is
    // treated as a deliberate separate repo.
    let output = work_dir.run_jj(["status"]);
    insta::assert_snapshot!(output.stderr, @"");

    // The diagnosis is available from `jj git colocation status`.
...
fn test_colocated_workspace_independent_heads() {
    ...
    fn test_independent_import(
        ...
        // git_head() was removed from revsets; an import resets the working
        // copy parent to the new Git HEAD, so @- observes the same thing.
        let output =
