...
fn test_git_init_external_but_git_dir_exists() {
    ...
    Hint: This Git repo is ignored by jj. If it is unintentional, remove it or move it out of the workspace.
    Initialized repo in "."
    [EOF]
    "#);
...
fn test_git_init_external_in_worktree_pointing_commondir() {
    ...
    let output = work_dir.run_jj(OP_LOG_COMPACT);
    insta::assert_snapshot!(output, @r#"
    @  4d34b8e2a79a new empty commit
    │  args: jj new
    ○  a50bdc8f53e8 import git head
    │  args: jj log -T '
    │      separate(" ",
    │        commit_id.short(),
    │        bookmarks,
