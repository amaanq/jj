...
fn diff_match_lines(
    ...
    if let (Some(left), Some(right)) = (lefts.as_resolved(), rights.as_resolved()) {
        let left: Merge<BString> = Merge::resolved(matcher.match_lines(left).collect());
        let right: Merge<BString> = Merge::resolved(matcher.match_lines(right).collect());
        let (selected, unselected) = match side {
            DiffMatchSide::Either => return Ok(left != right),
            DiffMatchSide::Left => (&left, &right),
            DiffMatchSide::Right => (&right, &left),
        };
        let diff = ContentDiff::by_line(itertools::chain(unselected, selected));
        Ok(diff.hunks().any(|hunk| {
            hunk.kind == DiffHunkKind::Different
                && matcher.match_lines(hunk.contents[1]).next().is_some()
        }))
    } else {
        ...
        let (selecteds, unselecteds) = match side {
            DiffMatchSide::Either | DiffMatchSide::Left => (lefts, rights),
            DiffMatchSide::Right => (rights, lefts),
        };
        let diff = ContentDiff::by_line(itertools::chain(&unselecteds, &selecteds));
        let mut hunks = files::conflict_diff_hunks(diff.hunks(), unselecteds.as_slice().len());
