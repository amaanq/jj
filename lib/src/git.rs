...
///
/// The HEAD read here is the backend repo's HEAD, so per-workspace semantics
/// require the backend to have been loaded with the workspace root (see
/// `GitBackend::load_at_workspace`), which makes it the workspace's worktree
/// HEAD. A repo loaded without workspace context reads the primary HEAD
/// regardless of `workspace_name`.
///
...
pub async fn import_head(
    ...
    let new_target = RefTarget::resolved(new_git_head_id);
    mut_repo.set_workspace_git_head(workspace_name, new_target.clone());

    // The legacy global git_head tracks the default workspace's HEAD so that
    // older jj versions and single-workspace repos keep working. Secondary
    // workspaces must not touch it, or it flip-flops between workspaces on
    // every command.
    if workspace_name == crate::ref_name::WorkspaceName::DEFAULT {
        mut_repo.set_git_head_target(new_target);
    }
...
pub async fn reset_head_at_workspace(
    ...
    // If a workspace path is provided and has a .git, open it as a worktree
    let git_repo = if let Some(ws_path) = workspace_path {
        ...
        if git_dir_path.exists() {
            ...
            // Don't fall back to the backend repo: it may belong to a
            // different workspace, and resetting its HEAD here would clobber
            // that workspace's state.
            gix::ThreadSafeRepository::open_opts(&git_dir_path, open_opts)
    ...
    if old_workspace_head != new_head_target {
        ...
        // The legacy global git_head tracks the default workspace's HEAD only;
        // see import_head().
        if workspace_name == crate::ref_name::WorkspaceName::DEFAULT {
