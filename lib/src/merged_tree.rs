...
impl From<TreeDiffEntry> for CopyHistoryTreeDiffEntry {
    ...
    fn from(tde: TreeDiffEntry) -> Self {
...
impl Stream for CopyHistoryDiffStream<'_> {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        loop {
            // First, check if we have newly-finished futures.
            if !self.pending.is_empty()
                && let Poll::Ready(Some(next)) = self.pending.poll_next_unpin(cx)
            {
                return Poll::Ready(Some(next));
            }
            ...
            let Ok(Diff { before, after }) = &next_tde.values else {
                self.pending.push_back(Box::pin(ready(next_tde.into())));
                continue;
            };
            ...
            match (before, after) {
                ...
                _ => self.pending.push_back(Box::pin(ready(next_tde.into()))),
            }
        }
    }
}
